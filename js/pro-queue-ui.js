...
function updateQueueUI() {
    ...
    dom.fileQueue.innerHTML = state.fileQueue.map(item => {
        ...
        if (item.status === 'pending') {
            actionButton = `<button class="text-red-500 hover:text-red-700 p-1" onclick="event.stopPropagation(); removeFromQueue('${escapeHtml(item.id)}')" title="移除">
            ...
            actionButton = `<button class="text-blue-500 hover:text-blue-700 p-1" onclick="event.stopPropagation(); retryItem('${escapeHtml(item.id)}')" title="重试">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
